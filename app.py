...
def home():
    nama = "natul"
    display=0
    test = pd.read_csv("dataset/u2.test.csv")
...
def data_tidak_setara():
  test = pd.read_csv("dataset/u2.test.csv")
  data_test = test.pivot(index = 'user_id', columns = 'item_id', values = 'rating').fillna(0)
  hasil_prediksi = pd.read_csv('dataset/Predict_F2K5_SDUB.csv')
  data_training = pd.read_csv('dataset/u2_base_container.csv')
  gt = pd.read_csv("dataset/u2_test_container.csv")
